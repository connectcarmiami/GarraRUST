...
>
> **Última atualização:** 2026-06-19 (local America/New_York) — **Anti-laundering + SSE guardado + teste humano Telegram PASS** (PR #3, binário `acbc32f1`, supera `ca1a981e`): `check_task`/`verify_task`/`get_task_result`/`cancel_task` mascaram ids inexistentes (anti-laundering — um id inventado roteado por tool não é mais "lavado"); o SSE de `/v1/chat/completions` envia só o texto guardado (gap anterior RESOLVIDO); `check_task` ganhou bloco AUDITORIA mascarado; monitor nunca infere `timed_out` de `timeout_at`. **Prova E2E humana**: heartbeat real `t-813d357b40de`, `message_id 369` → chat 7978617919, `chat_ok=true`, `scope=origin`. Ajustes locais de prompt/operação/segurança (PAPEL/AUTORIZAÇÃO, ATENDIMENTO US$/Miami, base `CONNECT_CAR_KB.md`). **Anterior (2026-06-18):** **Roteamento de heartbeat por chat de origem + contrato do output guard** (PR #2, binário `ca1a981e`): as notificações de delegação agora vão à conversa de ORIGEM (`7978617919`), não ao owner fixo — o binário injeta `garra_origin_chat_id` nas tools `delegation__*` (`tool_bridge.rs`), `_resolve_routing()` faz a origem vencer (owner só fallback), o monitor valida `chat_mismatch`. Prova E2E: `message_id` **312/313** no chat 7978617919. O **output guard** exibe ids reais de `ToolResult` no mesmo turno (smoke: `list_tasks` 15 ids, `ask_flash` `t-2ebba275fc28`, sem redação) e redige ids citados de memória em turnos posteriores — **regra operacional: IDs antigos devem ser RE-CONSULTADOS via `delegation__list_tasks`/`check_task` antes de citados**. **Anterior:** **Anti-hallucination output guard** entregue (tag `anti-hallucination-guard-2026-06-18`). Causa raiz: o modelo imprimia ids de tarefa inventados (`t-7f4e2c9a1b8d`/`t-8a3f5d7e2c9b`, `status: accepted`) sem chamar ferramenta — só prompt, sem trava de runtime. Correção: guarda em `garraia-agents` (`output_guard`) liga ids da resposta a evidência real de ferramenta e **redige** os não-verificados (`ID-NÃO-VERIFICADO`); camada Python (`ops/garra-delegation/`) com state machine + `succeeded`-exige-resultado + `verify_identifier` + trava de polling (**BLOCKED** em vez de loop) + ledger de entrega com `message_id`. **Prova E2E**: 2 notificações automáticas com `message_id` **283/284** persistidos. Deploy validado (binário sha `712625da…`); rollback via `~/garra-fix/backups/…/garraia.binary.bak` ou `GARRA_OUTPUT_GUARD=off`. Pendência cosmética: transitório de ~1s no streaming do Telegram antes da edição final já redigida. Ref: `docs/anti-hallucination-guard-2026-06-18.md`. **Anterior (2026-05-24):** docs/backlog sync: `TODO.md` criado como fila operacional obrigatória, GAR-493/ADR 0011 ✅ **Done** via PR [#492](https://github.com/michelbr84/GarraRUST/pull/492) (`95618d3`), `plans/README.md` sincronizado, e checklist GAR-603 reconciliada com evidência já presente em `Dockerfile`, `router.rs` e `docs/deployment-runpod.md`; smoke Docker/Runpod real permanece pendente. GarraMaxPower sincronizado: GAR-498 Skills MVP ✅ **Done** (PR [#488](https://github.com/michelbr84/GarraRUST/pull/488) `c65e099`), GAR-499 Agent Team MVP ✅ **Done** (PR [#490](https://github.com/michelbr84/GarraRUST/pull/490) `7e45ec5`). GAR-695 health run 23 docs ✅ **Done** via PRs [#493](https://github.com/michelbr84/GarraRUST/pull/493) / [#494](https://github.com/michelbr84/GarraRUST/pull/494). Anterior: GAR-679 SSE rate-limit per user ✅ **Done** (plan 0163); GAR-680 Audit-log of SSE chat subscriptions ✅ **Done** (PR [#463](https://github.com/michelbr84/GarraRUST/pull/463) `a972947`); GAR-496 Repo workflow seguro ✅ **Done** (PR #455 `1b7f04c`); GAR-495 ✅ **Done** (PR #453 `e5a2a08`); GAR-669 Slice 2 + GAR-500 Auto Dream ✅; GAR-372 embeddings scaffoldado; ADR 0010 Accepted + `garraia-learning`.
> **Owner:** @michelbr84
> **Equipe Linear:** GAR
> **Branch base:** `main`
